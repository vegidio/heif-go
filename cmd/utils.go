...
func encodeHeif(input, output string, options *heif.Options) (image.Image, error) {
...
func decodeHeif(input, output string) (image.Image, error) {
	ext := strings.ToLower(filepath.Ext(output))
	if !slices.Contains(ValidImageTypes, ext) {
		return nil, fmt.Errorf("invalid output file type: %s", ext)
	}
