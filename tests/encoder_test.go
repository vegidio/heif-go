...
func TestEncode(t *testing.T) {
	...
	t.Run("successful encode with custom quality", func(t *testing.T) {
		...
		var buf bytes.Buffer
		options := &heif.Options{Quality: 80}
		err := heif.Encode(&buf, img, options)
	...
	t.Run("encode with quality 0", func(t *testing.T) {
		...
		var buf bytes.Buffer
		options := &heif.Options{Quality: 0}
		err := heif.Encode(&buf, img, options)
	...
	t.Run("encode with quality 100", func(t *testing.T) {
		...
		var buf bytes.Buffer
		options := &heif.Options{Quality: 100}
		err := heif.Encode(&buf, img, options)
	...
	t.Run("invalid quality less than 0", func(t *testing.T) {
		...
		var buf bytes.Buffer
		options := &heif.Options{Quality: -1}
		err := heif.Encode(&buf, img, options)
	...
	t.Run("invalid quality greater than 100", func(t *testing.T) {
		...
		var buf bytes.Buffer
		options := &heif.Options{Quality: 101}
		err := heif.Encode(&buf, img, options)
	...
	t.Run("encode different image type NRGBA", func(t *testing.T) {
		...
		var buf bytes.Buffer
		err := heif.Encode(&buf, img, &heif.Options{Quality: 70})
	...
	t.Run("writer error", func(t *testing.T) {
		...
		err := heif.Encode(writer, img, nil)
...
func TestEncodeRealImage(t *testing.T) {
	t.Run("encode decoded HEIC image", func(t *testing.T) {
		...
		img, err := heif.Decode(reader)
		require.NoError(t, err)

		var buf bytes.Buffer
		err = heif.Encode(&buf, img, &heif.Options{Quality: 75})
		...
		decodedImg, err := heif.Decode(decodeReader)
		assert.NoError(t, err)
...
func TestEncodeRoundtrip(t *testing.T) {
	t.Run("encode and decode preserves dimensions", func(t *testing.T) {
		...
		// Encode
		var buf bytes.Buffer
		err := heif.Encode(&buf, original, &heif.Options{Quality: 90})
		require.NoError(t, err)

		// Decode
		reader := bytes.NewReader(buf.Bytes())
		decoded, err := heif.Decode(reader)
		require.NoError(t, err)
